...
// Modified by Philipp Suhrcke
// ANASOL Eulerian case for complex 3D geometries
// Geometry is given by a point cloud with labelled boundary and bulk points
...
void write_dt(std::string filename, double t,double dt){
    std::ofstream myFile(filename,std::ios_base::app);
    myFile <<t<<","<<dt<< "\n";
    myFile.close();
}
...
double Kt;
double lambda;
double zeta;       //zeta=-1 corresponds to all previous notations before 2018 paper of Julicher. In our code zeta +ve would mean contractile and negative would mean extensile.
double timeTOL;
double max_steady_tol;
double spacing;
bool neumann_BC;
...
constexpr int PID = 20;

void *vectorGlobal,*vectorGlobal_bulk,*vectorGlobal_boundary;
const openfpm::vector<std::string> PropNAMES={"00-Polarization","01-Velocity","02-Vorticity","03-ExternalForce","04-Pressure","05-StrainRate","06-Stress","07-MolecularField","08-VelocityRHS","09-FranckEnergyDensity","10-V_t","11-dV","12-dPol","13-P_old","14-div","15-delmu","16-Hperp","17-Hparallel","18-PolMag","19-tPolMag","20-ParticleID"};
typedef aggregate<VectorS<3, double>, VectorS<3, double>, double[3][3], VectorS<3, double>, double, double[3][3], double[3][3], VectorS<3, double>, VectorS<3, double>, double,VectorS<3, double>,VectorS<3, double>,VectorS<3, double>,VectorS<3, double>,double,double,VectorS<3, double>,double,double,double,int> ActiveGel3d;
...
openfpm::vector<aggregate<vect_dist_key_dx[2]>> CorrVec; 

// System, called each intermediate time step
// solves the force balance equations and calculate the time derivative of the polarization field
...
struct PolarEv
{
    ...
    void operator()( const state_type_3d_ofp &X , state_type_3d_ofp &dxdt , const double t ) const
    {
        ...
        // get latest polarization field from ODEint stepper
        Pol[x]=X.data.get<0>();
        Pol[y]=X.data.get<1>();
        Pol[z]=X.data.get<2>();
        ...
        if(neumann_BC){
            for (int j = 0; j < CorrVec.size(); ++j) 
            {
                ...
                auto p_in = CorrVec.get<0>(j)[1];
                Particles.getProp<Polarization>(p_out)=Particles.getProp<Polarization>(p_in);
            }
        }  

        // update ghosts because polarisation has changed
        ...
        // check deviation from unit magnitude
        double pmag=0;
        for (int j = 0; j < bulk.size(); j++) {
            auto p = bulk.get<0>(j);
        ...
        auto g_boundary = getV<ExtForce>(Particles_boundary);
        // auto g_down = getV<ExtForce>(Particles_boundary_down);
        auto V_boundary = getV<Velocity>(Particles_boundary);
        // auto V_down = getV<Velocity>(Particles_boundary_down);

        tt.start();
        Particles.ghost_get<Polarization>(SKIP_LABELLING);

        // define expression for better readability
        ...
        // compute Ericksen stress tensor
        sigma[x][x] =
        ...
        // RHS of the force balance equation
        ...
        P = 0;
        V_t = 0;
        DCPSE_scheme<equations3d3, vector_type> Solver(Particles);
        ...
        while (V_err >= V_err_eps && n <= nmax) {
            ...
            Solver.impose_x_ig(Boundary, V[x], vx);
            Solver.impose_x_ig(Boundary, V[y], vy);
            Solver.impose_x_ig(Boundary, V[z], vz);
            ...
            // compute L2 error between current and previous pressure correction iteration
            sum = 0;
            sum1 = 0;
            for (int j = 0; j < bulk.size(); j++) {
                auto p = bulk.get<0>(j);
            ...
            // check if error is converging or diverging 
        ...
        if (neumann_BC)
        {
            dPol[x] = (Hpar*Pol[x] - Hperp[z]*Pol[y] + Hperp[y]*Pol[z])/gama-nu*(Pol[x]*u[x][x]+Pol[y]*u[x][y]+Pol[z]*u[x][z]) + lambda*Pol[x]*delmu + (W[x][x]*Pol[x]+W[x][y]*Pol[y]+W[x][z]*Pol[z])-Pol[x]*div-(V[x]*dxpx+V[y]*dypx+V[z]*dzpx);
            dPol[y] = (Hpar*Pol[y] - Hperp[x]*Pol[z] + Hperp[z]*Pol[x])/gama-nu*(Pol[x]*u[y][x]+Pol[y]*u[y][y]+Pol[z]*u[y][z]) + lambda*Pol[y]*delmu + (W[y][x]*Pol[x]+W[y][y]*Pol[y]+W[y][z]*Pol[z])-Pol[y]*div-(V[x]*dxpy+V[y]*dypy+V[z]*dzpy);
            dPol[z] = (Hpar*Pol[z] - Hperp[y]*Pol[x] + Hperp[x]*Pol[y])/gama-nu*(Pol[x]*u[z][x]+Pol[y]*u[z][y]+Pol[z]*u[z][z]) + lambda*Pol[z]*delmu + (W[z][x]*Pol[x]+W[z][y]*Pol[y]+W[z][z]*Pol[z])-Pol[z]*div-(V[x]*dxpz+V[y]*dypz+V[z]*dzpz);
        } else
        {
        ...
        // pass derivatives to ODEint time stepper
...
// Observer, called at the end of each time step
// writes particles to output file and checks for steady state
template<typename DX,typename DY,typename DZ,typename DXX,typename DXY,typename DXZ,typename DYY,typename DYZ,typename DZZ>
struct CalcVelocity
{
    ...
    void operator() (state_type_3d_ofp &state, double t)
    {
        ...
        if(ctr%wr_at==0 || ctr==wr_f){
            // write current state of the particles to a file
            Particles.deleteGhost();
            Particles.write_frame("Polar3d", ctr,t,BINARY);
            Particles.ghost_get<0>();
        }

        // compute the maximum rate of change in the polarization field between the current and the last time step
...
int main(int argc, char* argv[])
{
    ...
    {   openfpm_init(&argc,&argv);
        ...
        // reading simulation and model parameters from parameters.txt file
        ...
        wr_at = Params.get(3);
        wr_f = tf/dt;
        V_err_eps = Params.get(4);
        adaptive = Params.get(5);       // 0: constant time stepper, 1: adaptive time stepper
        eta = Params.get(6);
        nu = Params.get(7);
        gama = Params.get(8);
        Ks = Params.get(9);
        Kb = Params.get(10);
        Kt = Params.get(11);
        lambda = Params.get(12);
        zeta = Params.get(13);
        timeTOL = Params.get(14);
        max_steady_tol = Params.get(15);
        spacing = Params.get(16);
        neumann_BC = Params.get(17);    // 0: Dirichlet, 1: Neumann
        ...
        // domain
        Box<3,double> box({-5,-5,-5},{5,5,5});
        // Box<3,double> box({0,0,0},{8,8,8});

        // Boundary conditions
        size_t bc[3]={NON_PERIODIC,NON_PERIODIC,NON_PERIODIC};

        // ghost, big enough to contain the interaction radius
        Ghost<3,double> ghost(rCut2+spacing/8.0);
        ...
        if (v_cl.rank()==0)
        {    
            // reading particle positions from csv file
            std::ifstream file;
            file.open(argv[2]);
            ...
            double PosZCSV;
            int flagSet;
            double PosXCSVNormal;
            double PosYCSVNormal;
            double PosZCSVNormal;

            double nLength = 0;
            size_t posctr=0;

            while ( file >> PosXCSV >> PosYCSV >> PosZCSV >> flagSet >> PosXCSVNormal >> PosYCSVNormal >> PosZCSVNormal)
            {

                double theta = (rand()/(float) RAND_MAX) * 2.0 * M_PI;
                double phi = (rand()/(float) RAND_MAX) * M_PI;

                Particles.add();
                Particles.getLastPos()[0] = PosXCSV;
                Particles.getLastPos()[1] = PosYCSV;
                Particles.getLastPos()[2] = PosZCSV;
                Particles.getLastProp<Polarization>()[x] = cos(theta)*sin(phi);
                Particles.getLastProp<Polarization>()[y] = sin(theta)*sin(phi);
                Particles.getLastProp<Polarization>()[z] = cos(phi);
                Particles.getLastProp<PID>() = 0;
                Particles.getLastSubset(0);

                if (flagSet == 1) 
                {
                    Particles.getLastProp<Polarization>()[x] = PosXCSVNormal;
                    Particles.getLastProp<Polarization>()[y] = PosYCSVNormal;
                    Particles.getLastProp<Polarization>()[z] = PosZCSVNormal;
        ...
        vector_dist_subset<3, double, ActiveGel3d> Particles_boundary(Particles,1);

        auto & bulk = Particles_bulk.getIds();
        auto & Boundary = Particles_boundary.getIds();

        // normalize polarization field (s.t. polarization at boundary is |p|=1)
        auto Pol = getV<Polarization>(Particles);
        auto PolMag = getV<POLMAG>(Particles);

        PolMag=sqrt(Pol[x]*Pol[x] + Pol[y]*Pol[y] + Pol[z]*Pol[z]);
        Pol=Pol/PolMag;

        // create new particles for Neumann BC
        size_t pctr=1;  // used to find particle pairs for Neumann BC
        if (neumann_BC){
            for (int i = 0; i < Boundary.size(); ++i)
            {
                auto p = Boundary.get<0>(i);
                double xp=Particles.getPos(p)[0]-0.5*spacing*Particles.getProp<Polarization>(p)[0];
                double yp=Particles.getPos(p)[1]-0.5*spacing*Particles.getProp<Polarization>(p)[1];
                double zp=Particles.getPos(p)[2]-0.5*spacing*Particles.getProp<Polarization>(p)[2];
                Particles.add();
                Particles.getLastPos()[x] = xp;
                Particles.getLastPos()[y] = yp;
                Particles.getLastPos()[z] = zp;
                Particles.getLastProp<Polarization>()[x] = Particles.getProp<Polarization>(p)[x];
                Particles.getLastProp<Polarization>()[y] = Particles.getProp<Polarization>(p)[y];
                Particles.getLastProp<Polarization>()[z] = Particles.getProp<Polarization>(p)[z];
                Particles.getLastSubset(1);
                Particles.getLastProp<PID>() = pctr;
                Particles.getLastProp<Pressure>() = 1;  // use pressure as flag to check if inner or outer layer
                Particles.getProp<PID>(p) = pctr;
                Particles.getProp<Pressure>(p) = 0;
                ++pctr;
            }
        } 
        ...
        int numberParticlesBoundary = Boundary.size();
        int numberParticlesBulk = bulk.size();
        v_cl.sum(numberParticlesBoundary);
        v_cl.sum(numberParticlesBulk);
        v_cl.execute();

        if (v_cl.rank()==0)
        {
            std::cout << "\n" << "----------------------------------------------------------" << "\n";
            std::cout << "Number of particles: " << numberParticlesBoundary + numberParticlesBulk << "\n";
            std::cout << "Number of bulk particles: " << numberParticlesBulk << "\n";
            std::cout << "Number of boundary particles: " << numberParticlesBoundary << "\n";
            if(neumann_BC) {
                std::cout << "Neumann boundary condition for polarization" << "\n";
            } else {
                std::cout << "Dirichlet boundary condition for polarization" << "\n";
            }  
        }

        // link particle pairs for Neumann BC using pctr
        if(neumann_BC){
            CorrVec.clear();
            for (int i = 0; i < Boundary.size(); ++i)
            {
                auto p_out = Boundary.get<0>(i);
                if (Particles.getProp<Pressure>(p_out) == 0) {
                    for(int j = 0; j < Boundary.size(); ++j) {
                        auto p_in = Boundary.get<0>(j);
                        if(Particles.getProp<PID>(p_out)==Particles.getProp<PID>(p_in) && p_out!=p_in){
                            
                            // USE Particles_boundary here!!!!
                            // if(Particles.getProp<PID>(p_boundary)==Particles.getProp<PID>(p_bulk)){
                            CorrVec.add();
                            CorrVec.get<0>(CorrVec.size()-1)[0]=p_out;
                            CorrVec.get<0>(CorrVec.size()-1)[1]=p_in;
                        }   
                    }
                }
            }
            int CorrVecLength = CorrVec.size();
            v_cl.sum(CorrVecLength);
            v_cl.execute();
            if (v_cl.rank()==0) {std::cout << "CorrVec length: " << CorrVecLength<< "\n";}
        }
        ...
        if (v_cl.rank()==0) {std::cout << "----------------------------------------------------------" << "\n";}

        Particles.deleteGhost();
        Particles.write_frame("Polar3d",BINARY);
        Particles.ghost_get<0>();
        ...
        boost::numeric::odeint::adaptive_adams_bashforth_moulton<2, state_type_3d_ofp,double,state_type_3d_ofp,double,boost::numeric::odeint::vector_space_algebra_ofp > abmA;

        size_t steps;
        if(adaptive){
            steps=boost::numeric::odeint::integrate_adaptive(boost::numeric::odeint::make_controlled(timeTOL,1e1*timeTOL,abmA),System,tPol,tim,tf,dt,CalcVelocityObserver);
        }
        else{
        ...
        Particles.deleteGhost();
        Particles.write_frame("Polar3d", wr_f,tf,BINARY);
        Particles.save("SaveEND"+std::to_string(int(tf)));
