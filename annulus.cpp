...
struct PolarEv
{
    ...
    void operator()( const state_type_3d_ofp &X , state_type_3d_ofp &dxdt , const double t ) const
    {
       ...
       auto Stokes1 = eta * (2*Dxx(V[x]) + Dxy(V[y]) + Dyy(V[x]) + Dxz(V[z]) + Dzz(V[x])) +
        ...
        Solver.impose(Stokes1, bulk, dV[0],  vx);
        Solver.impose(Stokes2, bulk, dV[1], vy);
        Solver.impose(Stokes3, bulk, dV[2], vz);
        Solver.impose(V[x], Boundary_up, 0, vx);
        ...
        while (V_err >= V_err_eps && n <= nmax) {
            ...
            Solver.impose_b(Boundary_up, 0, vx);
...
int main(int argc, char* argv[])
{
    ...
    {   openfpm_init(&argc,&argv);
        ...
        dmu = double(std::atof(argv[5]));
        //size_t grd_sz=int(sqrt(8.0*std::atof(argv[2])/125.0)+1);
        size_t grd_sz=int(std::atof(argv[6]));
        double iR = double(std::atof(argv[7]));
        double oR = double(std::atof(argv[8]));
        double boxsize = oR;
        double spacing_p = 2*oR / (grd_sz);
        double spacing_np = 2*oR / (grd_sz - 1);
        double box_z=2.78;
        size_t grd_sz_z = floor(box_z/spacing_p);//int(std::atof(argv[2]));
        const size_t sz[3] = {grd_sz+1, grd_sz+1, grd_sz_z};
        Box<3, double> box({-boxsize, -boxsize, 0}, {boxsize, boxsize, box_z});
        size_t bc[3] = {NON_PERIODIC, NON_PERIODIC, PERIODIC};
        double Lx = box.getHigh(0);
        double Ly = box.getHigh(1);
        double Lz = box.getHigh(2);
        double rCut = 2.1 * spacing_p;
        double rCut2 = 2.1 * spacing_p;
        int ord = 2;
        int ord2 = 2;
...
auto it = Particles.getGridIterator(sz);
        while (it.isNext()) {
            auto key = it.get();
            double x = -oR+key.get(0) * spacing_np;
            double y = -oR+key.get(1) * spacing_np;
            double z = key.get(2) * spacing_p;
            if(sqrt((x)*(x)+(y)*(y))<oR-3*spacing_np/6.0 && sqrt((x)*(x)+(y)*(y))>iR+3*spacing_np/6.0)
            {
                Particles.add();
        ...
        while (it2.isNext()) {
            ...
            double rad_p=sqrt((xp[0])*(xp[0])+(xp[1])*(xp[1]));
            double Px=(xp[x]),Py=(xp[y]);
            double P1 = Px/sqrt(Px*Px+Py*Py);
            double P2 = Py/sqrt(Px*Px+Py*Py);
            Particles.getProp<0>(p)[x]=-P2;
            Particles.getProp<0>(p)[y]=P1;
            Particles.getProp<0>(p)[z]=0;
            if(fabs(xp[0])<oR-2*spacing_np && fabs(xp[0])>iR+2*spacing_np && xp[1]<2*spacing_np && xp[1]>-2*spacing_np)
                {Particles.getProp<0>(p)[z]=1e-1;
                double mag=sqrt(Particles.getProp<0>(p)[0]*Particles.getProp<0>(p)[0]+
                    Particles.getProp<0>(p)[1]*Particles.getProp<0>(p)[1]+
                    Particles.getProp<0>(p)[2]*Particles.getProp<0>(p)[2]);
                Particles.getProp<0>(p)[0]=Particles.getProp<0>(p)[0]/mag;
                Particles.getProp<0>(p)[1]=Particles.getProp<0>(p)[1]/mag;
                Particles.getProp<0>(p)[2]=Particles.getProp<0>(p)[2]/mag;
                }
            ++it2;
        }
